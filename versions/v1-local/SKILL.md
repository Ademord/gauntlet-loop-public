---
name: gauntlet-loop
description: Creates short, paste-ready gauntlet-loop prompts and runs them when authorized. Combines a real reference, independent builders and critics, and evidence-backed acceptance checks. Done can optionally include CI, interactive and portable demos, clean public releases, README screenshots, update instructions, and project-specific regressions. Works for builds, writing, code, research, or design. Triggers on "$gauntlet-loop", "/gauntlet-loop", "gauntlet loop", "gauntlet this", "make a gauntlet prompt", or "loop until it beats X".
---
...
Run the builders and critics as parallel subagents.
```
...
Keep looping on each piece until the critic picks ours blind. Do not stop before that.
