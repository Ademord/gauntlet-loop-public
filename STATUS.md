...
The immediate work is [engineering readiness](research/program/readiness/README.md): ordinary-task outcomes suggest possible gaps, the existing fifty milestones help choose what to investigate, and deterministic/native checks assess a proposed capability before paid comparisons. Observations and causal hypotheses remain separate; a failure does not establish that a new feature is needed. `python tools/readiness.py list` currently selects managed access to existing validation; the observer is reused. P001 failed acceptance after one repair ($4.7742095 CLI-reported execution usage; full research cost unknown), and no pipeline changes were installed.
