...
| Passive observer | Codex native request/tool/completion capture verified; Claude failure lifecycle captured, successful tools blocked by usage credits. Captured human contacts can be classified separately from explicit resolution. Selected evidence manifests can be exported from private snapshots | [observer](ledger/OBSERVER.md), [native validation](research/program/materials/observer-native-validation.json) |
