...
| `hooks/` | Pre-commit guard: `check_staged.py` refuses staged text with personal paths, identity, secrets, or a term from your local `tools/private_terms.local.json` | `git config core.hooksPath tools/hooks` |
...
| `paired_study/` | Paired-study harness: survey repositories for tasks, generate arm prompts that differ only in the flag, run a pair, analyze pairs with exact tests | see [paired_study/README.md](paired_study/README.md) |
