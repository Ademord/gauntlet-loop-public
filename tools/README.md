...
| `paired_study/` | Paired-study harness: `survey_repos.py` finds candidate tasks, `mint_mutations.py` mints them, `make_arms.py` writes the two arm prompts, `run_pair.py` runs one pair headlessly behind a login preflight, `run_series.py` walks the ready pairs, `analyze_pairs.py` applies the committed exact tests, `report_pairs.py` writes the look report | see [paired_study/README.md](paired_study/README.md) |
