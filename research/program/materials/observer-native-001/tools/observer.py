...
FIELDS = ('turn_id', 'prompt_id', 'model', 'permission_mode', 'tool_name', 'tool_use_id',
...
def validate_config(config: dict):
    ...
    for value in config.values():
        if isinstance(value, list):
            ...
            _text(value, True)
def _register(conn, session_id, task_id, config):
    """Enroll within the caller's transaction (also used for one-shot host enrollment)."""
...
def register_session(path, session_id, task_id, config: dict):
    ...
    try:
        with _db(path) as conn:
            _register(conn, session_id, task_id, config)
...
def ingest_hook(path, payload: dict):
    ...
    with _db(path) as conn:
        ...
        if event in TOOL_EVENTS and metadata.get('tool_use_id'):
            identity = _json([session_id, event, metadata['tool_use_id'],
                              metadata.get('turn_id') or metadata.get('prompt_id')])
        status = 'stable_tool_identity' if identity else 'unknown'
        if event == 'UserPromptSubmit' and metadata.get('prompt_id'):
            identity = _json([session_id, event, 'prompt_id', metadata['prompt_id']])
            status = 'stable_prompt_identity'
        try:
...
def session_report(path, session_id):
    ...
    return {
        ...
        'events': {'total': sum(counts.values()), 'by_event': counts,
                   ...
                   'tool_ends': counts.get('PostToolUse', 0),
                   'tool_failures': counts.get('PostToolUseFailure', 0), 'identityless': unknown},
        'deduplication': {'status': 'partial' if unknown else 'stable_event_identities_only',
