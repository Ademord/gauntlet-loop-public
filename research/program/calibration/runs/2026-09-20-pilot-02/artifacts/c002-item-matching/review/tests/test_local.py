...
class LocalTests(unittest.TestCase):
    ...
    def test_large_finite_amount_still_matches(self):
        # Finite but very large amounts must not overflow the Decimal
        # quantize context and silently become ineligible.
        predicted = [{"name": None, "amount": 1e300}]
        reference = [{"name": None, "amount": 1e300}]
        result = match_items(predicted, reference)
        self.assertEqual(result["pairs"], [(0, 0)])
