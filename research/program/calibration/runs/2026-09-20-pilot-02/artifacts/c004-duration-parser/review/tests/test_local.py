...
class LocalTests(unittest.TestCase):
    ...
    def test_non_finite_total_does_not_crash(self):
        huge = "9" * 400
        self.assertEqual(parse_duration(f"PT{huge}H"), "unknown")
        self.assertEqual(parse_duration(f"PT{huge}M"), "unknown")
        self.assertEqual(parse_duration(f"PT{huge}S"), "unknown")
        self.assertEqual(parse_duration(f"about {huge} hours"), "unknown")
        self.assertEqual(parse_duration(f"{huge} min"), "unknown")
