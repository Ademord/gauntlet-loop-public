...
class LocalTests(unittest.TestCase):
    ...
    def test_equal_instants_across_different_offsets(self):
        utc = {"project": "p", "run_id": "r", "updated_at": "2026-01-01T10:00:00+00:00", "state": "utc"}
        offset = {"project": "p", "run_id": "r", "updated_at": "2026-01-01T15:30:00+05:30", "state": "offset"}
        self.assertEqual(select_latest([utc, offset]), [offset])
        self.assertEqual(select_latest([offset, utc]), [utc])

    def test_non_string_or_missing_run_id_or_project_skipped(self):
        records = [
            {"project": None, "run_id": "r", "updated_at": "2026-01-01T10:00:00Z"},
            {"project": "p", "run_id": None, "updated_at": "2026-01-01T10:00:00Z"},
            {"project": True, "run_id": "r", "updated_at": "2026-01-01T10:00:00Z"},
            {"project": ["p"], "run_id": "r", "updated_at": "2026-01-01T10:00:00Z"},
        ]
        self.assertEqual(select_latest(records), [])

    def test_different_run_ids_same_project_are_distinct(self):
        r1 = {"project": "p", "run_id": "one", "updated_at": "2026-01-01T10:00:00Z"}
        r2 = {"project": "p", "run_id": "two", "updated_at": "2026-01-01T09:00:00Z"}
        self.assertEqual(select_latest([r1, r2]), [r1, r2])

    def test_invalid_identity_rows_do_not_affect_first_appearance_order(self):
        r1_invalid = {"project": "p", "run_id": "r", "updated_at": "not-a-date", "state": "first-invalid-ts"}
        other = {"project": "q", "run_id": "s", "updated_at": "2026-01-01T09:00:00Z", "state": "other"}
        r1_valid = {"project": "p", "run_id": "r", "updated_at": "2026-01-01T10:00:00Z", "state": "first-valid-ts"}
        skipped = {"project": "", "run_id": "r", "updated_at": "2026-01-01T12:00:00Z", "state": "skipped"}
        result = select_latest([r1_invalid, other, r1_valid, skipped])
        self.assertEqual(result, [r1_valid, other])

    def test_malformed_and_offset_without_colon_timestamps(self):
        malformed = {"project": "p", "run_id": "r", "updated_at": "2026-01-01T10:00:00+05:30Z", "state": "bad"}
        no_colon = {"project": "p", "run_id": "r", "updated_at": "2026-01-01T10:00:00+0000", "state": "ok"}
        self.assertEqual(select_latest([malformed, no_colon]), [no_colon])

    def test_input_list_not_mutated(self):
        records = [
            {"project": "p", "run_id": "r", "updated_at": "2026-01-01T10:00:00Z"},
            {"project": "q", "run_id": "s", "updated_at": "2026-01-01T09:00:00Z"},
        ]
        original_ids = [id(r) for r in records]
        original_len = len(records)
        select_latest(records)
        self.assertEqual(len(records), original_len)
        self.assertEqual([id(r) for r in records], original_ids)
