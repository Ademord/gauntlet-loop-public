...
CAPS = {'build': .75, 'self_check': .75, 'reviewer': .25, 'repair': .50}
            ...
            with tempfile.TemporaryDirectory(prefix='gauntlet-cal-') as td:
                ...
                for branch in order:
                    ...
                    if branch == 'self_check':
                        ...
                        review = controller.call(task_id, 'reviewer', task_prompt(spec) + '\nYou are the independent reviewer. '
                                                 'Inspect the current code read-only. Identify concrete requirement violations with '
                                                 'reproducible examples, or state none found. Do not invent a required change. '
                                                 'The harness ran the visible tests:\n' + visible.stdout + visible.stderr, work)
